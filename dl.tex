...
\Hrule
  ...
  \begin{center}
    \begin{tabular}[t]{l|l}
     ...
     \texttt{GNU_PROPERTY_X86_FEATURE_2_USED} & \texttt{0xc0000003} \\
     \texttt{GNU_PROPERTY_X86_FEATURE_2_NEEDED} & \texttt{0xc0000004} \\
    \end{tabular}
  \end{center}
\Hrule
\end{table}
...
\begin{description}
 \item[GNU_PROPERTY_X86_ISA_1_USED] Its \code{pr_data} field contains
   ...
   hardware is optional.  Its absence in an x86 ELF binary implies that
   any x86 instruction sets may be used.  If a bit in the \code{pr_data}
   field is unset, the corresponding x86 instruction set isn't used.
   A bit in the output \code{pr_data} field is set if  it is set in
   any relocatable input \code{pr_data} fields and
   \texttt{GNU_PROPERTY_X86_ISA_1_USED} is present in all relocatable
   input files.
 ...
 \item[GNU_PROPERTY_X86_FEATURE_2_USED] Its \code{pr_data} field contains
   a 4-byte integer.  The x86 processor features indicated by the
   corresponding bits are used in program.  Their support in the
   hardware is optional.  Its absence in an x86 ELF binary implies that
   any x86 processor features may be used.  If a bit in the \code{pr_data}
   field is unset, the corresponding processor feature isn't used.
   A bit in the output \code{pr_data} field is set if  it is set in
   any relocatable input \code{pr_data} fields and
   \texttt{GNU_PROPERTY_X86_FEATURE_2_USED} is present in all relocatable
   input files.
 \item[GNU_PROPERTY_X86_FEATURE_2_NEEDED] Its \code{pr_data} field
   contains a 4-byte integer.  The x86 processor features indicated by
   the corresponding bits are used in program and they must be supported
   by the hardware.  A bit in the output \code{pr_data} field is
   set if it is set in any relocatable input \code{pr_data} fields.
\end{description}
...
\Hrule
  ...
  \begin{center}
    \begin{tabular}[t]{l|l}
      \multicolumn{1}{c}{Name} & \multicolumn{1}{c}{Value} \\
      \hline
     \texttt{GNU_PROPERTY_X86_ISA_1_486} $^\dagger$ & \texttt{1U << 0} \\
     ...
     \texttt{GNU_PROPERTY_X86_ISA_1_FMA} & \texttt{1U << 18} \\
     \texttt{GNU_PROPERTY_X86_ISA_1_AVX512_4FMAP} & \texttt{1U << 19} \\
     \texttt{GNU_PROPERTY_X86_ISA_1_AVX512_4VNNIW} & \texttt{1U << 20} \\
     \texttt{GNU_PROPERTY_X86_ISA_1_AVX512_BITALG} & \texttt{1U << 21} \\
     \texttt{GNU_PROPERTY_X86_ISA_1_AVX512_IFMA} & \texttt{1U << 22} \\
     \texttt{GNU_PROPERTY_X86_ISA_1_AVX512_VBMI} & \texttt{1U << 23} \\
     \texttt{GNU_PROPERTY_X86_ISA_1_AVX512_VBMI2} & \texttt{1U << 24} \\
     \texttt{GNU_PROPERTY_X86_ISA_1_AVX512_VNNI} & \texttt{1U << 25} \\
    \cline{1-2}
    \multicolumn{2}{p{13cm}}{\small $^\dagger$ This bit should always
    be set when \texttt{GNU_PROPERTY_X86_ISA_1_USED} is used.}\\
    \end{tabular}
  \end{center}
\end{table}
